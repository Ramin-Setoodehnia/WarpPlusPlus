...
# Version: 7.2 (Final Stable - UI Logic Completely Rewritten to Prioritize User Input)
#
# This script will:
...
echo "Starting Warp+ All-in-One Installer v7.2..."
sleep 2
        ...
        if (data && data.mode) {
            ...
            setInterval(() => {
                ...
                callAPI('?action=status', newData => {
                    ...
                    // If the server config has changed externally, resync everything
                    if (newData.mode !== serverState.mode || newData.reconnect_enabled !== serverState.reconnect_enabled) {
                        serverState = deepCopy(newData);
                        uiState = deepCopy(newData);
                        showNotification("Settings were changed externally. UI has been updated.", 4000);
                    }
                    updateUI();
                });
            }, 7000);
